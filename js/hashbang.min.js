...
this._xhr.responseType=a;this._xhr.addEventListener("timeout",()=>{this._ready=!0;this._targets.forEach((a)=>a.removeAttribute("loading"));this.dispatch("timeout",this._targets)},!1);this._xhr.addEventListener("load",()=>this._load(),!1);this._xhr.addEventListener("error",()=>this._error(),!1);this._xhr.addEventListener("progress",(a)=>{this.dispatch("progress",a.loaded,a.total)},!1)}_load(){this._ready=!0;this._targets.forEach((a)=>a.removeAttribute("loading"));400>this._xhr.status?this.dispatch("loaded",
this._xhr.response,this._targets):this.dispatch("error",this._xhr.response||{code:this._xhr.status,message:this._xhr.statusText},this._targets)}_error(){this._ready=!0;this._targets.forEach((a)=>a.removeAttribute("loading"));this.dispatch("error",{code:523,message:"Disconnected"},this._targets)}request(a,b,c,e){c=c||null;e=e||null;console.info("Client: %s %s",a,b,c);this._targets.forEach((a)=>a.removeAttribute("loading"));this._targets.clear();this._xhr.abort();"GET"===a&&null!==c&&(b+=`?${g.query(c)}`);
this._xhr.open(a,b,!0);this._xhr.setRequestHeader("Accept",this._accept);"GET"!==a&&null!==c&&(c=JSON.stringify(c),this._xhr.setRequestHeader("Content-Type","application/json"));null!==e&&this._targets.push(...e);this._targets.forEach((a)=>a.addAttribute("loading"));this._xhr.send("GET"!==a?c:null);this._ready=!1}static encode(a){return c.encodeURIComponent(a)}static decode(a){return c.decodeURIComponent(a)}static query(a){return Object.keys(a).map((b)=>`${b}=${g.encode(a[b])}`).join("&")}}class r extends e{get open(){return this.element.hasAttribute("open")}set open(a){a?
this.element.addAttribute("open"):this.element.removeAttribute("open")}constructor(a){super();this.define("element",a)}show(){this.open=!0}close(){this.open=!1}}c.define("HB",{});c.HB.define("uuid",()=>"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(a){let b=16*Math.random()|0;"y"===a&&(b=b&3|8);return b.toString(16)}));c.HB.define("Event",k);c.HB.define("EventTarget",e);c.HB.define("Timer",n);c.HB.define("Template",p);c.HB.define("Router",l);c.HB.define("Client",g);c.HB.define("Showable",
r)})(this);